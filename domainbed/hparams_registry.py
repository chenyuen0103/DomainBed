...
def _hparams(algorithm, dataset, random_seed, model_type='ViT-S'):
    ...
    if algorithm in ['DANN', 'CDANN']:
        ...
        _hparam('grad_alpha', 5000, lambda r: 10 ** r.uniform(1, 3))
        _hparam('hess_beta', 500, lambda r: 10 ** r.uniform(2, 4))
        # _hparam('penalty_anneal_iters', 2500, lambda r: int(r.uniform(0., 5000.)))
        _hparam('penalty_anneal_iters', 1500, lambda r: int(np.random.choice([500, 1500, 2500, 3500, 4500])))
...
def random_hparams(algorithm, dataset, seed, model_type='ViT-S'):
    return {a: c for a, (b, c) in _hparams(algorithm, dataset, seed, model_type).items()}
